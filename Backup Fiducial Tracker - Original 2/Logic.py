...
indigo.server.log('starting detection first time')
trackingoff = 0
...
try:
 ...
 #              indigo.server.log('justopened set to 1')
            ...
            if trackingoff == 1: 
                ...
                indigo.server.log('starting detection in shirt trackingoff')
  ...
  #             indigo.server.log('shirt_front detected') 
        ...
        if myVar.value == "true":
 #          indigo.server.log("drawer_2_open variable is 'true'")   
            time.sleep(delaytimer)                                  # delay every cycle (minimal)

            pantstimer += 1

            if justopened_2 == 0:
                indigo.server.log('justopened_2 is 0 - hopefully only once')
                justopened_2 = 1                                    # play audio -- take out shirt -- open the shirt
 #              indigo.server.log('justopened set to 1')
                for i in range(1,3):                                # *** UPDATE SOUND FILES
                    play_audio(shirt_files + str(i))                # *** UPDATE SOUND FILES PANTS
                    time.sleep(2)

            if trackingoff == 1: 
                trackingoff = 0
                tracking = tuio.Tracking("100.10.100.10", 3333)     # TURN THIS BACK ON
                indigo.server.log('starting detection tracking pants')
  ...
  #             indigo.server.log('shirt_front detected') 
                pantsf += 1                                         # *** Add logic here to incement iterations of detection until confirmed
                pantsf = 3                                          # *** FORCE detct on first detect // update logic for multiple checks
                if pantsf > 2:
                    indigo.server.log('Pants_front confirmed')
                    play_audio(completed_files + congrats)
                    time.sleep(4)                                   # pause for audio
                    play_audio('Pants/closeDrawer')                 # UPDATE *** audio -- close drawer
                    #Set drawer1 light to off
                    #Set drawer2 light to on                        
            ...
            if pants_back in fid_ids:    
                if pantsbackf < 3:                                  # number of times asked to turn around
                    pantsbackf += 1
                    indigo.server.log('pants_back detected')
                    play_audio(shirt_files + shirt_backwards)       # UPDATE AUDIO
                    # *** IMPORTANT RESET ALERT ***                 # this seems to be working well
                    tracking.stop()                                 
                    trackingoff = 1
                    time.sleep(5)                                   # delay to let pwd turn shirt around
                    ...
                    pantstimer = maxtimer                           # if number of tries is too much go to "call for help"

            if pantstimer == maxtimer:                              # shirt front not detected "call for help"
                 indigo.server.log('pants -- needs help')
                 play_audio(completed_files + help_inbound)
                 keepgoing = 0


        myVar = indigo.variables["drawer_3_open"]                   # check if Drawer 1 is open
        if myVar.value == "true":
 #          indigo.server.log("drawer_1_open variable is 'true'")   
            time.sleep(delaytimer)                                  # delay every cycle (minimal)
            ...
            if justopened_3 == 0:
                indigo.server.log('justopened_3 is 0 - hopefully only once')
                justopened_3 = 1                                    # play audio -- take out shirt -- open the shirt
 #              indigo.server.log('justopened set to 1')
                for i in range(1,3):
                    play_audio(shirt_files + str(i))                # UPDATE
                    time.sleep(2)

            if trackingoff == 1: 
                trackingoff = 0
                tracking = tuio.Tracking("100.10.100.10", 3333)     # TURN THIS BACK ON
                indigo.server.log('starting detection boxer trackingoff')
  ...
  # CHECK IF FORWARDS/ BACKWARDS  - assumes drawer is left open during shirt donning
            if boxer_front in fid_ids:  
  #             indigo.server.log('shirt_front detected') 
                boxerf += 1                                         # *** Add logic here to incement iterations of detection until confirmed
                boxerf = 3                                          # *** FORCE detct on first detect // update logic for multiple checks
                if boxerf > 2:
                    indigo.server.log('Boxer_front confirmed')
                    play_audio(completed_files + congrats)
                    time.sleep(4)                                   # pause for audio
                    play_audio('Shirt/closeDrawer')                 # UPDATE -- audio -- close drawer
                    #Set drawer1 light to off
                    #Set drawer2 light to on                        
                    keepgoing = 0                                   # *** UPDATE this once other cloths are in place
                else: 
            ...
            if boxer_back in fid_ids:    
                if boxerbackf < 3:                                  # number of times asked to turn around
                    boxerbackf += 1
                    indigo.server.log('boxer_back detected')
                    play_audio(shirt_files + shirt_backwards)       #UPDATE
                    # *** IMPORTANT RESET ALERT ***                 # this seems to be working well
                    tracking.stop()                                 
                    trackingoff = 1
                    time.sleep(5)                                   # delay to let pwd turn shirt around
                    ...
                    boxertimer = maxtimer                           # if number of tries is too much go to "call for help"

            if boxertimer == maxtimer:                              # shirt front not detected "call for help"
                 indigo.server.log('boxer -- needs help')
                 play_audio(completed_files + help_inbound)
                 keepgoing = 0
